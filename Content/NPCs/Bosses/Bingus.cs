...
using UltimateBoss.Content.Items;
using UltimateBoss.Content.Projectiles;
...
namespace UltimateBoss.Content.NPCs.Bosses
{
	...
	public class Bingus : ModNPC
	{
		...
		public bool SpawnedGauntlet = false;
		...
		public override void SetStaticDefaults()
		{
			...
			{
				//CustomTexturePath = "ExampleMod/Assets/Textures/Bestiary/MinionBoss_Pre",
		...
		public override void SetDefaults()
		{
			...
			if (!Main.dedServ)
			{
				Music = MusicLoader.GetMusicSlot(Mod, "Assets/Music/BingusTheme");
			}
		}
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement> {
				...
				new FlavorTextBestiaryInfoElement("The most Powerful being of all time.")
			});
		}
/*
...
*/
		...
		public override void FindFrame(int frameHeight)
		{
			...
			if (SecondStage)
			{
				...
				if (Shooting)
                {
					NPC.frame.Y = firstshooting * frameHeight;
                }
...
*/
		...
		private void SpawnGlove()
		{
			...
			int index = NPC.NewNPC(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y - 100, ModContent.NPCType<ThanosGauntlet>(), NPC.whoAmI);
			
			NPC ThanosNPC = Main.npc[index];

			// Now that the minion is spawned, we need to prepare it with data that is necessary for it to work
			// This is not required usually if you simply spawn NPCs, but because the minion is tied to the body, we need to pass this information to it

			if (ThanosNPC.ModNPC is ThanosGauntlet minion)
			{
				// This checks if our spawned NPC is indeed the minion, and casts it so we can access its variables
				minion.ParentIndex = NPC.whoAmI; // Let the minion know who the "parent" is
				minion.PositionIndex = 1; // Give it the iteration index so each minion has a separate one, used for movement
			}
		...
		private void ShootGun(Player player)
        {
			...
			var entitySource = NPC.GetSource_FromAI();
			if (AttackCounter == 50 || AttackCounter == 55 || AttackCounter == 60 || AttackCounter == 65 || AttackCounter == 70)
			{
				Shooting = true;
				for (int i = 0; i < 2; ++i)
				{
		...
		private void DoSecondStage_SpawnEyes(Player player)
		{
			...
			if (NPC.HasValidTarget && SecondStageTimer_SpawnEyes == 0 && Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				int type = ModContent.ProjectileType<BlueFlame>();
				int damage = NPC.damage / 2;
				var entitySource = NPC.GetSource_FromAI();
